...
template node['nginx_front']['lb_conf'] do
  source "#{node['pathes']['cookbooks_root']}/nginx/front/templates#{node['nginx_front']['lb_conf']}.erb"
  variables(
    listen: node['nginx_front']['listen'],
    server_name: node['nginx_front']['server_name'],
    server1: node['nginx_web']['server_name1'],
